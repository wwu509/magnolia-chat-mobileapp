...
const MessageList: React.FC = () => {
  const [searchFilterListing, setSearchFilterListing] = useState<
    MessageItemProps[]
  >([]);
  const getChatListing = useCallback(async () => {
    const {data} = await axiosConfig.get(CHAT_API.GET_CHAT_LIST);
    return data;
  }, []);

  const {data: chatListing} = useQuery<MessageItemProps[]>({
    queryKey: ['chats'],
  ...
  const searchFilter = useCallback(
    async (params: {
      ...
      const {data} = await axiosConfig.post<MessageItemProps[]>(
  ...
  const searchFilterSuccess = useCallback(async (data: MessageItemProps[]) => {
    setSearchFilterListing(data);
  }, []);

  const SearchFilterAPIMutate: UseMutationResult<
    MessageItemProps[],
    MessageAPIAxiosError,
    {filter?: string; search?: string}
  > = useMutation({
    mutationFn: searchFilter,
    onSuccess: searchFilterSuccess,
    onError: (error: MessageAPIAxiosError) => {
      const errorMessage =
  ...
  const handleSearchFilter = useCallback(
    (params: {filter?: string; search?: string}) => {
      SearchFilterAPIMutate.mutate(params);
    },
    [SearchFilterAPIMutate],
  );
  ...
  return (
    <SafeAreaView className="flex flex-col rounded-none w-full  max-w-[90%] m-auto">
      ...
      <ScrollView
        ...
        <SortBy handleSearch={handleSearchFilter} />
        {(
          (searchFilterListing?.length > 0 && searchFilterListing) ||
          chatListing ||
          messages
        )?.map((message, index) => <MessageItem key={index} {...message} />)}
      </ScrollView>
    </SafeAreaView>
  );
};
